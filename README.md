# Nick (vanzan01) 👋

**Perth, Australia** 🇦🇺 • **AI Toolsmith**

I build practical AI developer infrastructure: context engineering, agent orchestration, and small tools that remove friction.

> Open-source here is personal work, not client work.

![Go](https://img.shields.io/badge/-Go-00ADD8?style=flat&logo=go&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![Claude](https://img.shields.io/badge/-Claude-CC785C?style=flat&logo=anthropic&logoColor=white)

---

## What I Build

- **Context engineering & memory systems** (turn “chat” into durable engineering)
- **Multi-agent orchestration** (coordination patterns that scale)
- **MCP servers & integrations** (bridge tools to LLM workflows)
- **Developer utilities** (small, sharp tools that ship)

## Featured Projects

![Total Stars](https://img.shields.io/github/stars/vanzan01?style=social&label=Total%20Stars)
![Profile Views](https://komarev.com/ghpvc/?username=vanzan01&style=social)

### 🧠 Context Engineering & Memory Systems
- **[cursor-memory-bank](https://github.com/vanzan01/cursor-memory-bank)** — Persistent AI memory with visual process maps. VAN/PLAN/CREATIVE lifecycle for structured AI development
- **[Claude-Code-MindPalace](https://github.com/vanzan01/Claude-Code-MindPalace)** — Never write commit messages again. Auto-checkpoint, auto-squash into professional git history

### 🤖 Multi-Agent Orchestration
- **[claude-code-sub-agent-collective](https://github.com/vanzan01/claude-code-sub-agent-collective)** — Hub-and-spoke coordination through Claude Code. Context engineering research in action. One of the very first long running agents.

### 🔌 MCP Servers & Integrations
- **[cline.code](https://github.com/vanzan01/cline.code)** — OpenAI-compatible API bridge for Claude.Code via MCP
- **[atomic-writer-mcp](https://github.com/vanzan01/atomic-writer-mcp)** — Atomic file operations. Built to test MCP. Launch month after watching AI overwrite my Obsidian notes one too many times
- **[poe2scout-mcp](https://github.com/vanzan01/poe2scout-mcp)** — POE2 trading intelligence: 11 specialized tools for currency analysis & arbitrage

### 🧰 Developer Tools
- **[markdown-viewer](https://github.com/vanzan01/markdown-viewer)** — Professional markdown viewer for Windows/macOS. Perfect for sharing AI outputs with non-technical staff
- **[claude-code-reclaimr](https://github.com/vanzan01/claude-code-reclaimr)** — Intelligent file organization assistant. 7/30-day archiving while you sleep
- **[yt-whisper-analyzer](https://github.com/vanzan01/yt-whisper-analyzer)** — YouTube transcription & keyword analysis via Whisper/Groq
- **[whisper-remote](https://github.com/vanzan01/whisper-remote)** — Trigger Whisper Flow from your phone. Walk around, dictate, hit enter — stay in flow

### 🎮 Side Projects
- **[elden-ring-interactive-guide](https://github.com/vanzan01/elden-ring-interactive-guide)** — Interactive quest tracker for Elden Ring

---

## My Stack

| Phase | Tools |
|-------|-------|
| **Requirements & Brainstorming** | Wispr Flow, Whisper Remote |
| **Development** | Claude Code, OpenCode, Codex, Gemini (images) |
| **Language** | Go is my go-to recently |
| **Hosting** | Cloudflare |
| **Second Brain** | Fabric |
| **Research** | Perplexity API |

---

## Stats

![GitHub Contribution Graph](https://ghchart.rshah.org/vanzan01)

---

## Recognition

- **3,500+ GitHub stars** in under a year, built under this handle
- **Top 0.01%** of GitHub repositories globally
- Featured at [AI Coding Dojo](https://www.docswell.com/s/nid/57RXWV-2025-06-19-211920) meetups in Japan
- cursor-memory-bank: 400+ forks, 15% fork ratio!

### Media
- [YouTube: cursor-memory-bank walkthrough](https://www.youtube.com/watch?v=dF4uCZAY1tk)
- [YouTube: Context Engineering deep dive](https://www.youtube.com/watch?v=oj44Vrh0XyM)

---

## Latest from the Blog (Coming Soon)

- [How I Reached GitHub's Top 0.01%](https://blog.vanzan01.org/posts/2026/how-i-reached-github-top-001-percent/)
- [The Side Hustle That Mattered: My AI Journey from 2017 to Now](https://blog.vanzan01.org/posts/2026/the-side-hustle-that-mattered/)
- [Making Your Blog AI-Agent Friendly with llms.txt](https://blog.vanzan01.org/posts/2026/making-your-blog-ai-agent-friendly/)
- [Whisper Remote: Stay in Flow While Talking to AI](https://blog.vanzan01.org/posts/2026/whisper-remote/)

---

## Philosophy

> Work counts. Credentials are optional. Ship tools that solve real problems — the stars follow.

> If it's missing, build it. Ship it fast!

> Work on solving hard problems.

> The pipeline is the product. The UI is optional.

---

## Work With Me

I take on small, high-leverage consulting engagements around agent pipelines, context engineering, and AI tooling.

## Connect

- Blog: [blog.vanzan01.org](https://blog.vanzan01.org)
- GitHub: You're here!
